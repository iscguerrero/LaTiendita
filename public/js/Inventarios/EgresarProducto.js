$(document).ready(function () {
	...
	var tipos = ObtenerCatMovimientos({ tipo: 'S' });
	...
	$('#tablaEgresos tbody').on('click', 'button.borrar', function () {
		$('#tablaEgresos').bootstrapTable('remove', { field: 'cve_cat_producto', values: [selectedItem] });
	});
	...
	$('#agregar').click(function () {
		if ($('#cve_cat_producto').val() == '') {
			swal({
				html: '<h3>Debes capturar la información del producto a egresar</h3>',
		...
		if ($('#cantidad').val() == '' || $('#cantidad').val() == 0) {
			swal({
				html: '<h3>Debes capturar la cantidad de producto a egresar</h3>',
				showConfirmButton: true,
				type: 'info'
			});
			return false;
		...
		if ($flag == false) {
			...
			$('#tablaEgresos').bootstrapTable('append', row);
		}
		...
		$.ajax({
			...
			success: function (data) {
				if (data.bandera == false) {
					...
					$.notify({
						message: 'El egreso de los productos se realizó con éxito'
					}, {
							type: 'success'
						});
					setProducto({ cve_cat_producto: '', codigo_de_barras: '', value: '', departamento: '', marca: '' });
					$('#tablaEgresos').bootstrapTable('load', []);
					swal.close();
				}
			}
		});
